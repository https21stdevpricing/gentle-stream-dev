...
import { ArrowRight, MapPin, Phone, Mail, Gem, Users, Truck, ShieldCheck, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="bg-background" data-testid="about-page">
      ...
      <div className="pt-[48px]">
        {/* Hero — clean Apple white */}
        <section className="text-center py-24 md:py-36 px-6 bg-background">
          <div className="container-sw">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <h1 className="apple-headline mb-4" style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)' }}>
                About Stone World.
              </h1>
              <p className="apple-subhead text-base md:text-lg max-w-2xl mx-auto leading-relaxed">
        ...
        <section className="py-20 md:py-28 px-6 bg-background">
          <div className="container-sw">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
              <h2 className="apple-headline text-2xl md:text-3xl">Our Journey.</h2>
            </motion.div>
              ...
              {TIMELINE.map((t, i) => (
                <motion.div key={t.year} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <div className={`flex-1 ${i % 2 === 0 ? 'md:text-right' : ''}`}>
                    <p className="text-foreground font-semibold text-lg tracking-tight">{t.year}</p>
                  ...
                  <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-foreground border-4 border-background" />
          ...
          <div className="container-wide">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                className="rounded-2xl overflow-hidden" style={{ aspectRatio: '4/3' }}>
        ...
        {/* Clients — Apple dark section */}
        <section className="py-16 md:py-24 px-6 bg-[#1d1d1f] text-center" data-testid="clients-section">
          <div className="container-sw">
            <h2 className="font-semibold text-white text-2xl tracking-tight mb-8">Trusted by India's Finest.</h2>
            <div className="flex flex-wrap gap-2.5 justify-center">
              {clients.map((c, i) => (
                <motion.span key={i} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
                  transition={{ delay: i * 0.04 }}
                  className="px-4 py-2 bg-white/5 rounded-full text-white/50 text-sm hover:text-white/80 transition-colors cursor-default">
                  {c}
                </motion.span>
              ))}
            </div>
        ...
        <section className="py-20 md:py-28 px-6 bg-background">
          <div className="container-sw text-center">
            ...
            <div className="flex flex-wrap gap-3 justify-center mb-10">
              <Link to="/support" className="btn-primary text-sm" data-testid="about-contact-btn">
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 text-muted-foreground text-sm">
              <span className="flex items-center gap-2"><Phone size={14} /> {siteInfo.phone1 || '+91 9377521509'}</span>
              <span className="flex items-center gap-2"><Mail size={14} /> {siteInfo.email || 'Stoneworld1947@gmail.com'}</span>
              <span className="flex items-center gap-2"><MapPin size={14} /> {siteInfo.address || 'Near Dantali Gam, Gujarat'}</span>
            </div>
          </div>
        </section>
      </div>
