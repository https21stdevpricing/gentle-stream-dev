...
function SkeletonCard() {
  return (
    <div className="animate-pulse">
      <div className="rounded-2xl bg-[hsl(var(--muted))] mb-3" style={{ aspectRatio: '3/4' }} />
      <div className="h-2.5 bg-[hsl(var(--muted))] rounded-full w-1/4 mb-2" />
      <div className="h-3.5 bg-[hsl(var(--muted))] rounded-full w-3/4 mb-1" />
      <div className="h-2.5 bg-[hsl(var(--muted))] rounded-full w-1/2" />
    </div>
  );
}
  ...
  return (
    <div className="bg-background min-h-screen" data-testid="products-page">
      ...
      <div className="pt-[48px]">
        {/* Hero — minimal & clean */}
        <section className="text-center py-14 md:py-20 px-6 bg-background mandala-pattern">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative flex-1 max-w-xs">
                ...
                <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                  ...
                  className="w-full pl-10 pr-9 py-2 bg-[hsl(var(--muted))] border-none rounded-full text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-foreground/10 transition-all" />
                {search && (
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide flex-1" data-testid="category-filters">
                <button onClick={() => setCategory('')} data-testid="filter-all"
                  className={`shrink-0 px-4 py-1.5 rounded-full text-[12px] font-medium transition-all duration-300 ${
                    !selectedCategory ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground hover:bg-[hsl(var(--muted))]'
                  }`}>
                  All
                </button>
                {categories.filter(c => c.active).slice(0, 8).map(c => (
                  <button key={c.id} onClick={() => setCategory(selectedCategory === c.name ? '' : c.name)}
                    ...
                    className={`shrink-0 px-4 py-1.5 rounded-full text-[12px] font-medium transition-all duration-300 ${
                      selectedCategory === c.name ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground hover:bg-[hsl(var(--muted))]'
                    }`}>
                    {c.name}
                  </button>
                ))}
              ...
              <div className="hidden md:flex items-center gap-1 ml-auto shrink-0">
                <button onClick={() => setGridCols(3)} className={`p-1.5 rounded-lg transition-colors ${gridCols === 3 ? 'bg-[hsl(var(--muted))] text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
                  <Grid3X3 size={15} />
                </button>
                <button onClick={() => setGridCols(4)} className={`p-1.5 rounded-lg transition-colors ${gridCols === 4 ? 'bg-[hsl(var(--muted))] text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
                  <LayoutGrid size={15} />
                </button>
            ...
            <div className={`grid grid-cols-2 md:grid-cols-3 ${gridCols === 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-5 md:gap-6`}>
              {products.map((p, i) => (
                <motion.div key={p.id}
                  ...
                  <ProductCard product={p} onClick={setSelectedProduct} linkTo={`/products/${p.slug}`} />
                </motion.div>
              ))}
            </div>
          )}
